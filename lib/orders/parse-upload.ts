...
function normalizeRow(input: UploadRow) {
  ...
  const alt = normalized.altPhone;
  const cp = normalized.customerPhone;
  const cpEmpty = cp === undefined || cp === null || String(cp).trim() === "";
  const altOk = alt !== undefined && alt !== null && String(alt).trim() !== "";
  if (cpEmpty && altOk) {
    normalized.customerPhone = typeof alt === "string" ? alt.trim() : String(alt);
  }
