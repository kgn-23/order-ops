    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className={`${showMobileKpis ? "grid" : "hidden"} gap-2 md:grid md:grid-cols-3 xl:grid-cols-7`}>
          ...
          <div className="rounded-md border p-3"><p className="text-xs text-muted-foreground">Today</p><p className="text-lg font-semibold">{counts.today}</p></div>
          ...
          <div className="rounded-md border p-3"><p className="text-xs text-muted-foreground">Callback Requested</p><p className="text-lg font-semibold">{counts.callbackRequested}</p></div>
          <div className="rounded-md border p-3"><p className="text-xs text-muted-foreground">Confirmed</p><p className="text-lg font-semibold">{counts.confirmed}</p></div>
          <div className="rounded-md border p-3"><p className="text-xs text-muted-foreground">Cancelled</p><p className="text-lg font-semibold">{counts.cancelled}</p></div>
        ...
        >
          ...
          <div className="flex flex-col gap-2">
            ...
            <select id="outcome" name="outcome" defaultValue={filters.outcome} className="h-9 rounded-md border bg-background px-3 text-sm">
              <option value="ALL">All</option><option value="NO_ANSWER">No Answer</option><option value="CALLBACK_REQUESTED">Callback Requested</option><option value="CONFIRMED">Confirmed</option><option value="DELAYED">Delayed</option><option value="CANCELLED">Cancelled</option><option value="INVALID_NUMBER">Invalid Number</option><option value="OTHER">Other</option>
            </select>
          </div>
          <div className="flex flex-col gap-2">
              ...
              <div key={row.id} className="rounded-xl border bg-card p-3 shadow-sm">
                <div className="mb-2 flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <Badge variant="outline">{row.orderStage ?? "—"}</Badge>
                  </div>
                </div>
              ...
              {rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <tr key={row.id} className="border-t">
                    ...
                    <td className="px-3 py-2"><Badge variant="secondary">{row.outcome}</Badge></td>
                    <td className="px-3 py-2"><Badge variant="outline">{row.orderStage ?? "—"}</Badge></td>
