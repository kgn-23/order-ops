...
import { createCallerTeamSchema, createTeamUserSchema, updateTeamUserSchema } from "@/app/server/contracts";
...
export async function createTeamUser(input: unknown): Promise<TeamActionResult> {
  ...
  const { name, email, phone, role, callerTeamId } = parsed.data;
  ...
  try {
    const initialPasswordHash = await hash(randomUUID(), 12);
...
export async function updateTeamUser(input: unknown): Promise<TeamActionResult> {
  ...
  const { userId, name, email, phone, isActive, role, callerTeamId } = parsed.data;
  const normalizedEmail = normalizeEmail(email);
